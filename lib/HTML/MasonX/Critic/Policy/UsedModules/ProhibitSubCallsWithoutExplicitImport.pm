...
sub violates {
    ...
    foreach my $block ( @blocks ) {
        ...
        my @constants = HTML::MasonX::Critic::Inspector::Query::Factory::PerlCode->find_constant_declarations( $block );
        foreach my $constant ( @constants ) {
            $available_subs{ $constant->symbol } = 'constant';
        }

        my @subroutines = HTML::MasonX::Critic::Inspector::Query::Factory::PerlCode->find_subroutine_declarations( $block );
