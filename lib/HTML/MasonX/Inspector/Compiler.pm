...
use HTML::MasonX::Inspector::PerlSource::CriticViolation;
    ...
    if ( defined $methods && keys %$methods ) {
        return map
            HTML::MasonX::Inspector::Compiler::Method->new(
                ...
                body => HTML::MasonX::Inspector::PerlSource->new(
                    code => \($methods->{$_}->{body})
                ),
            ), keys %$methods
    }
...
sub get_blocks {
    ...
    if ( $self->{_compiler}->{main_compile}->{body} ) {
        $blocks{main} = [
            HTML::MasonX::Inspector::PerlSource->new(
                code => \($self->{_compiler}->{main_compile}->{body})
            )
        ];
    }

    $blocks{once}    = [ map HTML::MasonX::Inspector::PerlSource->new(code => \$_), @{ $blocks->{once}    } ] if @{ $blocks->{once}    };
    $blocks{init}    = [ map HTML::MasonX::Inspector::PerlSource->new(code => \$_), @{ $blocks->{init}    } ] if @{ $blocks->{init}    };
    $blocks{filter}  = [ map HTML::MasonX::Inspector::PerlSource->new(code => \$_), @{ $blocks->{filter}  } ] if @{ $blocks->{filter}  };
    $blocks{cleanup} = [ map HTML::MasonX::Inspector::PerlSource->new(code => \$_), @{ $blocks->{cleanup} } ] if @{ $blocks->{cleanup} };
    $blocks{shared}  = [ map HTML::MasonX::Inspector::PerlSource->new(code => \$_), @{ $blocks->{shared}  } ] if @{ $blocks->{shared}  };
...
sub get_violations {
    ...
    my @violations = map HTML::MasonX::Inspector::PerlSource::CriticViolation->new(
        violation => $_
    ), @original;
